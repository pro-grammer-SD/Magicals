...
if "user" not in st.session_state and cookies.get("access_token"):
    try:
        ...
        if session and getattr(session, "user", None):
            ...
            alert(title=f"🎉 Welcome back, {session.user.email}!", key="welcome_back")
    except Exception:
...
if "user" in st.session_state:
    with card(key="logged_in_card"):
        ...
        alert(title=f"You're logged in as {st.session_state.user['email']}", key="logged_in_alert")
...
with card(key="auth_card"):
    ...
    if submit:
        if not email or not password:
            alert(title="⚠️ Please fill all fields.", key="missing_fields")
        else:
            try:
                if mode == "Sign Up":
                    res = supabase.auth.sign_up({"email": email, "password": password})
                    if res.user:
                        alert(title="✅ Account created! Please verify your email before logging in.", key="signup_ok")
                        st.balloons()
                else:
                ...
                alert(title=f"⚠️ {msg}", key="auth_error")
